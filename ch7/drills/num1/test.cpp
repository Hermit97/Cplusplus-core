...
// Exercise 04: class Symbol_table
// Exercise 05: return print when newline found
// Exercise 06: help
// Exercise 07: change quit and help commands
// Exercise 08: complete grammar
// Exercise 09: already done in exercise 4
// Exercise 12: already done in exercise 2
...
public:
  ...
  Token(char ch, string n)
      : kind(ch), name(n) {} // make a Token from a char and a string
};
...
Token Token_stream::get() {
  ...
  while (isspace(ch)) {
    ...
    cin.get(ch);//gets the token like the result
  }
  ...
  default:
    if (isalpha(ch)) {
      ...
      if (s == conkey)
        return Token(con); // constant keyword
      if (s == sqrtkey)
        return Token(square_root); // square root keyword
      if (s == powkey)
        return Token(power); // power function keyword
      if (s == helpkey)
        return Token(help); // help keyword
      if (s == quitkey)
...
void calculate() {
  while (cin)
    try {
      ...
      if (t.kind == help)
        print_help(); // print help instructions
      else {
        if (t.kind == quit)
          return;
        ts.putback(t);
