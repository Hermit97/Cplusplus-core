...
// Clear input until c or '\n' is found
{
    ...
    // DRILL 3. I find more useful for ignore to consider new line as limit for
    // ignore. This causes user to be prompted again in case of input a name
    // not yet defined (in the original you must insert a print command for
    // prompt to be recovered). As it is, >> operator doesn't do the work and I
    // must use con.get() to detect the newline char.
	char ch{' '};
    while (ch != c && ch != '\n')
        ch = cin.get();
    return;
...
double primary()
{
	...
	case '-':   // Negative signed numbers
		return - primary();
	// DRILL 3. Not an error but it seems stupid to include negative signed
	// numbers and don't do the same with positive signed numbers
...
double declaration()
{
	...
	Token t = ts.get();
	// DRILL 3. Get rid of magic constant 'a' with the already defined constant
	// name. It implies to change previous inner function variable name to
	// var_name and avoid conflicts.
